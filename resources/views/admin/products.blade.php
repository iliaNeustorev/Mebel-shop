    ...
    <div class = 'container'>
        ...
        <form enctype="multipart/form-data" action="{{ route('add_product') }}" method="post">
            ...
            <div class="mb-3">    
                ...
                <textarea required class="form-control w-50" name ='description' row = '3' placeholder="Введите описание продукта"></textarea>
            </div>
            ...
            <tbody>
                    ...
                    @foreach ($products as $product)
                        ...
                        <tr>
                            ...
                            <td>{{ $product->price }}</td>
                            <td><a href="{{ route('admin_get_product', $product->category->id) }}" title="Вывести список продуктов категории">{{ $product->category->name }}</a></td>
                            <td>
                                <a href="{{ route('admin_get_product',  [$product->category->id, $product->id]) }}" title="Редактировать продукт"><img class="avatar" src="{{ asset('storage/img/products') }}/{{ $product->picture }}" alt="{{ $product->name }}"></a>
                            </td>
